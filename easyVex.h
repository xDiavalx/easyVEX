...
function int[] uniquearray(int numbers[]) {
	...
	numbers = sort(numbers); // sort ascending to simplify the elimination of duplicates

	append(clean, numbers[0]); // the first element is always unique 

	for (int i = 1; i<len(numbers); i++) {
		if (clean[temp]<numbers[i]) { //if the number is bigger than the previously added, add it
			append(clean, numbers[i]);
